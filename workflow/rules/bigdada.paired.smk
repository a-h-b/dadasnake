...
rule dada_control:
    input:
        ...
        expand("stats/multiqc_{step}_{direction}.{run}_report.html",step=['1','filtered'],run=samples.run.unique(),direction=['fwd','rvs']) 
    output:
        "dada.done"
    shell:
        """
...
rule big_multiqc:
    ...
    output:
        directory('stats/multiqc_{step}_{dir}.{run}_report_data'),
        "stats/multiqc_{step}_{dir}.{run}_report.html"
    threads: 1
    resources:
        runtime = "8:00:00",
        mem = config['normalMem']
    conda: ENVDIR + "fastqc.yml"
    log: "logs/multiqc_{step}_{dir}.{run}.log"
    message: "multiqc_1: Collecting fastQC on {wildcards.step} {wildcards.dir} reads for {wildcards.run}."
    shell:
        """
        export LC_ALL=en_GB.utf8
        export LANG=en_GB.utf8
        multiqc -n {output[1]} {input}/*{wildcards.dir}_fastqc.* >> {log} 2>&1
        """
